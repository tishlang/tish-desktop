...
Core runtime (`cargo:tishlang_app`, `@tishlang/tish-app-api`, BrokerCore, caps, bridges) has **zero** dependency on lattish or `@tishlang/tish-desktop-ui-theme`.
...
If lattish adds `NativeSurface` / `WebSurface`, they must be thin aliases over the table above and must **never** be required by `cargo:tishlang_app` or `@tishlang/tish-app-api`. RN `View` stays a **layout** adapter for DOM/webview UI, not a surface/window API.
